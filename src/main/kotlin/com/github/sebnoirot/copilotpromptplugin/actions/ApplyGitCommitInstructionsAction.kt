...
/**
 * Action to manually apply the Git commit instructions to the current project.
 */
class ApplyGitCommitInstructionsAction : AnAction(), DumbAware {

    override fun actionPerformed(e: AnActionEvent) {
        val project = e.project ?: return

        val promptService = CopilotPromptService.getInstance(project)
        val (success, virtualFile, wasCreated) = promptService.applyGitCommitInstructions()
    ...
    private fun showNotification(project: Project, success: Boolean, virtualFile: VirtualFile?, wasCreated: Boolean) {
        ...
        if (success) {
            ...
            val message = if (wasCreated) {
                ...
                "The Git commit instructions file has been updated."
            }

            val notification = notificationGroup.createNotification(
                "Git commit instructions applied",
                message,
            ...
            notificationGroup.createNotification(
                "Failed to apply Git commit instructions",
                "There was an error creating or updating the Git commit instructions file.",
